...
import { Instagram, Send, Menu, X, CheckCircle2, ArrowLeft, ChevronRight, MapPin, ShieldCheck } from 'lucide-react';
...
function App() {
  ...
  const RNAVT = "13143"; 
  const NIF = "500000000"; // Substitui pelo NIF real da empresa
  const NOME_FISCAL = "DADOS_DA_EMPRESA_LDA"; // Substitui pelo nome oficial da Lda
  const MORADA = "MORADA_COMPLETA, CODIGO-POSTAL"; // Substitui pela morada da sede
    ...
    <nav className="bg-white/95 backdrop-blur-md sticky top-0 z-[100] border-b border-stone-100">
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-6">
          <a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="text-[#103D3B] hover:opacity-70">
            <Instagram size={20} />
          </a>
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F1] font-sans">
      ...
      {page === 'home' && (
        <header className="relative h-[80vh] flex items-center justify-center text-center px-6 overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <p className="text-white/80 uppercase tracking-[0.4em] text-[10px] font-bold">Especialistas em Experiências de Luxo</p>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            {destinosData.map((d) => (
              <div key={d.id} className="cursor-pointer group" onClick={() => setSelectedDestino(d)}>
                <div className="h-[450px] overflow-hidden rounded-sm shadow-xl mb-6">
                  <img src={d.img} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" alt={d.title} />
                </div>
      ...
      {selectedDestino && (
        <div className="animate-in fade-in">
          <div className="h-[50vh] relative">
            ...
            <button onClick={() => setSelectedDestino(null)} className="absolute top-6 left-6 bg-white/80 p-2 rounded-full hover:bg-white transition-all shadow-lg">
          ...
          <div className="max-w-5xl mx-auto py-16 px-6 grid md:grid-cols-3 gap-12">
              ...
              <div className="space-y-4 pt-6 border-t border-stone-200">
                {selectedDestino.detalhes.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <CheckCircle2 size={18} style={{ color: azulMar }} />
            ...
            <div className="bg-white p-8 rounded-sm shadow-2xl border-t-4 flex flex-col justify-center text-center space-y-6" style={{ borderTopColor: azulMar }}>
              <h4 className="font-bold uppercase text-xs tracking-widest" style={{ color: azulMar }}>Pedido de Informação</h4>
              <p className="text-xs text-stone-500 leading-relaxed italic">Consulte as nossas condições exclusivas para este destino.</p>
              <button onClick={() => {setFormData({...formData, destino: selectedDestino.title}); setPage('contacto'); setSelectedDestino(null);}} className="w-full py-5 text-white font-bold uppercase tracking-widest text-[10px] transition-all hover:brightness-110 flex items-center justify-center gap-2" style={{ backgroundColor: azulMar }}>
                Solicitar Detalhes <ChevronRight size={14}/>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ORÇAMENTO */}
      {page === 'contacto' && (
        <section className="py-20 px-6 max-w-3xl mx-auto animate-in slide-in-from-bottom-4">
          <h2 className="text-3xl font-light italic text-center mb-12" style={{ color: azulMar }}>Formulário de <span className="font-bold not-italic uppercase">Contacto</span></h2>
          <form onSubmit={handleFormSubmit} className="bg-white p-8 md:p-12 shadow-2xl rounded-sm space-y-6">
            <input type="text" placeholder="Destino" value={formData.destino} className="w-full border-b py-3 outline-none font-bold bg-stone-50 px-3" style={{ borderBottomColor: azulMar, color: azulMar }} onChange={(e) => setFormData({...formData, destino: e.target.value})} />
            <input type="text" placeholder="Nome" className="w-full border-b py-3 outline-none focus:border-[#103D3B]" required />
            <input type="email" placeholder="Email" className="w-full border-b py-3 outline-none focus:border-[#103D3B]" required />
            <textarea placeholder="Mensagem..." className="w-full border p-4 h-32 outline-none focus:border-[#103D3B] text-sm resize-none"></textarea>
      ...
      {/* FOOTER LEGAL REFORÇADO */}
      <footer className="py-20 bg-white border-t border-stone-200 px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12 text-center md:text-left">
          <div className="space-y-4">
             <div className="flex flex-col">
              <span className="text-lg font-light tracking-[0.3em]" style={{ color: azulMar }}>LATITUDE C</span>
              <span className="text-[10px] font-bold text-stone-400 uppercase">Agência de Viagens</span>
            </div>
          ...
          <div className="space-y-2 text-[10px] text-stone-500 uppercase tracking-widest font-bold">
            <div className="flex items-center justify-center md:justify-start gap-2">
              <ShieldCheck size={14} style={{ color: azulMar }} />
              <span>RNAVT Nº {RNAVT}</span>
            </div>
            <div className="flex items-center justify-center md:justify-start gap-2">
              <MapPin size={14} style={{ color: azulMar }} />
              <span>{MORADA}</span>
            </div>
          </div>
          ...
          <div className="space-y-2 text-[9px] text-stone-400 uppercase leading-relaxed">
            <p>{NOME_FISCAL}</p>
            <p>NIF: {NIF}</p>
            <p>© 2026 LATITUDE C. Todos os direitos reservados.</p>
          </div>
        </div>
      </footer>
    </div>
